...
function printClassDetails(type, context) {
    if (type.kind !== pg_schema_introspect_1.ClassKind.OrdinaryTable &&
        type.kind !== pg_schema_introspect_1.ClassKind.PartitionedTable) {
        throw new Error('printClassDetails only supports ordinary or partitioned tables at the moment.');
    }
    ...
    const InsertParameters = context.printer.pushTypeDeclaration({ type: 'insert_parameters', name: type.className }, (identifierName, file) => [
        ...
        ...type.attributes
            .map((attribute) => [
            ...getAttributeComment(attribute),
            `  ${attribute.attributeName}${optionalOnInsert(attribute, context)}: ${getAttributeType(type, attribute, context, file)}`,
        ])
            .reduce((a, b) => [...a, ...b], []),
        `}`,
    ]);
...
function optionalOnInsert(attribute, context) {
    if (!attribute.notNull)
        return '?';
...
function isDynamicDefault(defaultValue) {
    let inString = false;
